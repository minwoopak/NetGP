...
class GetData():
    ...
    def _stat_drug(self, drug_cell_df):
        print("#" * 50)
        drug_num = drug_cell_df['DRUG_ID'].value_counts().shape[0]  # Unique #Drugs
        print('#\t #Unique Drugs：{}'.format(drug_num))
        min_cell = min(drug_cell_df['DRUG_ID'].value_counts())    # Min #Paired Cell
        max_cell = max(drug_cell_df['DRUG_ID'].value_counts())    # Max #Paired Cell
        mean_cell = np.mean(drug_cell_df['DRUG_ID'].value_counts())  # Mean #Paired Cell
        print('#\t Min #Paired Cells: {}，\n\t Min #Paired Cells:{}，\n\t Min #Paired Cells:{}'.format(
            min_cell, max_cell, mean_cell))

    def _split(self,df,col,ratio,random_seed):

        col_list = df[col].value_counts().index # unique drugs list
        train_data = pd.DataFrame()
        test_data = pd.DataFrame()

        for instatnce in col_list:
            sub_df = df[df[col] == instatnce]  # fetch response pairs that involves current drug
            sub_df = sub_df[['DRUG_ID', 'COSMIC_ID','TCGA_DESC', 'LN_IC50']]
            ## Split each drug's response samples by certain ratio: MixSplit(Drug exists in both Train&Test) ##
        ...
        print('#' * 50)
        print('#\t #total response data：{}'.format(df.shape[0]))
        print('#\t Split by{}，Train {}%，Test {}%'.format(col,(1-ratio),ratio))
        print('#\t #Train Pairs：{}'.format(train_data.shape[0]))
        print('#\t #Test Pairs： {}'.format(test_data.shape[0]))
