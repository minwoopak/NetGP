...
# ====== AGW: Model Definition ====== #
class DrugResponseNet_NP_Concat(nn.Module):
    def __init__(self, drug_embed_net, #cell_embed_net, 
                 np_embed_net,
                 attention_net, 
                 fc_in_dim=18115+1024, fc_hid_dim=[512, 512], dropout=0.5):
        super(DrugResponseNet_NP_Concat, self).__init__()
    ...
    def forward(self, drug_x, cell_x, np_x):
        ...
        context_vector = torch.cat((drug_x, np_x, cell_x), dim=1)
    ...
    def get_embedding(self, drug_x, cell_x, np_x):
        # === embed drug === #
        drug_x = self.drug_embed_net(drug_x)
        ## === embed cell line === #
        #cell_x = self.cell_embed_net(cell_x)
        ...
        return drug_x, np_x, c_weights, context_vector, output
...
parser.add_argument('--model_name', type=str, default='agw_netgp')
...
# === Model Setting === #
args.drug_embed_dim = 128

args.dropout = 0.8
args.response_unit_hid_dim = [512, 256, 128]
...
for var1 in list_var1:
    for var2 in list_var2:
        ...
        args.cell_line_dim = train_set.cell_exprs_df.shape[1] - 2
        ...
        drug_embed_net = EmbedNet(fc_in_dim=args.drug_dim,
                                  ...
                                  dropout=0.2)
        np_embed_net = EmbedNet(fc_in_dim=args.np_score_dim,
                                  fc_hid_dim=args.drug_embed_hid_dim, 
                                  embed_dim=args.drug_embed_dim, 
                                  dropout=0.2)
        attention_net =  ResponseUnit(drug_dim=args.drug_embed_dim, 
                                      fc_hid_dim=args.response_unit_hid_dim, 
                                      output_dim=args.cell_line_dim, 
                                      dropout=0.2)
        model =          DrugResponseNet_NP_Concat(drug_embed_net, np_embed_net, attention_net, 
                                         fc_in_dim=args.drug_embed_dim+args.drug_embed_dim+args.cell_line_dim,
