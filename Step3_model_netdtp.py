...
class data_process_loader(data.Dataset):
	def __init__(self, list_IDs, labels, drug_df, rna_df, np_df):
		'Initialization'
		self.labels = labels
	...
	def __getitem__(self, index):
		...
		v_n = np.array(self.np_df.iloc[index])
		v_p = np.array(self.rna_df.iloc[index])
		y = self.labels[index]
		
		return v_d, v_p, v_n, y
...
class Classifier(nn.Sequential):
    def __init__(self, model_drug, model_gene, model_np):
        ...
        dims = [self.input_dim_drug + self.input_dim_gene + self.input_dim_gene] + self.hidden_dims + [1]
        self.predictor = nn.ModuleList([nn.Linear(dims[i], dims[i + 1]) for i in range(layer_size)])

    def forward(self, v_D, v_P, v_N):
        # each encoding
        v_D = self.model_drug(v_D)
        v_P = self.model_gene(v_P)
        v_N = self.model_np(v_N)
        
        # === Integration Method: Weighting === #
        #v_P = v_P + v_N
        
        # concatenate and classify
        v_f = torch.cat((v_D, v_N, v_P), 1)
        for i, l in enumerate(self.predictor):
...
class DeepTTC_NetDTP_Concat:
    def __init__(self, modeldir, device):
        model_drug = transformer(device=device)
        model_gene = MLP(input_dim_gene=978, device=device)
        model_np = MLP(input_dim_gene=1157, device=device)
        self.model = Classifier(model_drug, model_gene, model_np)
        self.device = device
    ...
    def test(self,datagenerator,model):
        ...
        y_pred = []
        model.eval()
        for i,(v_drug, v_gene, v_np, label) in enumerate(datagenerator):
            score = model(v_drug, v_gene, v_np)
    ...
    def train(self, train_drug, train_rna, train_np, val_drug, val_rna, val_np):
        lr = 1e-4
        decay = 0
        BATCH_SIZE = 512
        train_epoch = 100
        ...
        training_generator = data.DataLoader(data_process_loader(
            train_drug.index.values, train_drug.Label.values, train_drug, train_rna, train_np), **params)
        validation_generator = data.DataLoader(data_process_loader(
            val_drug.index.values, val_drug.Label.values, val_drug, val_rna, val_np), **params)
        ...
        for epo in range(train_epoch):
            for i, (v_d, v_p, v_n, label) in enumerate(training_generator):
                # v_d = v_d.float().to(self.device)
                score = self.model(v_d, v_p, v_n)
