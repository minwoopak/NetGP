...
from Step3_model import DeepTTC, logging
...
parser.add_argument('--workdir', type=str, default="/data/project/minwoo/Drug_recommendation/NetDTP")
# args.workdir = "/data/project/minwoo/Drug_recommendation"
parser.add_argument('--datadir', type=str, default='/data/project/minwoo/Drug_recommendation/NetDTP/Data')
# args.datadir = os.path.join(args.workdir, 'Data')
parser.add_argument('--inputdir', type=str, default='/data/project/minwoo/Drug_recommendation/NetDTP/Data/model_input')
# args.inputdir = os.path.join(args.datadir, 'model_input', 'data_v4')
parser.add_argument('--model_setting', type=str, default='DeepTTA')
# args.model_setting = 'DeepTTA'
parser.add_argument('--split_type', type=str, default='drug', choices=['cell','drug','both','mix'])
# args.split_type = 'drug'
parser.add_argument('--response_type', type=str, default='drug', choices=['IC50', 'AUC'])
# args.response_type = 'IC50'
...
args.outdir = os.path.join(args.workdir, 'New_Framework_v4', 'Results_add', args.exp_name)
...
for cv in cv_list:
    ...
    net = DeepTTC(modeldir=modeldir, device=device)
    net.train(train_drug=traindata, train_rna=train_rnadata,
              val_drug=validdata, val_rna=valid_rnadata)
    net.save_model()
    print("Model Saveed :{}".format(modelfile))

    y_label, y_pred, mse, rmse, pearson, p_val, spearman, s_p_val, CI = net.predict(testdata, test_rnadata)
    print("Test Result: \n")
